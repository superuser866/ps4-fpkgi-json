...
update_json() {
    ...
    jq --arg k "$key" --argjson v "$value" '. + {($k): $v}' "$json_file" > tmp.json && mv tmp.json "$json_file"
}
...
cleanup_json() {
    ...
    original_keys=$(jq -r 'keys[]' "$json_file")
	...
	full_path=${key#$SERVER_URL}
        ...
        if [ -f "$full_path" ]; then
            ...
            echo "Record deleted (not found) in $1 : $full_path"
            deleted_keys+="$key"$'\n'
	    #cJsonRm = $((cJsonRm + 1))
        fi
    ...
    jq --argjson kept_keys "$(echo "$kept_keys" | jq -R -s -c 'split("\n") | map(select(length > 0))')" '
        to_entries | 
        map(select(.key as $key | $kept_keys | index($key))) | 
        from_entries' "$json_file" > tmp.json && mv tmp.json "$json_file"

    #echo "PKGs removed from $1 -> $cJsonRm"
}
...
while read -r pkg; do
    ...
    version=$(docker exec "$CONTAINER_NAME" grep "VERSION" /workspace/tmpfile | awk -F'=' '{print $2}' | tr -d ' ')
	...
	if [[ -e "./_img/$title_id.PNG" ]]; then
	    ...
	    # If PIC0 is empty, try PIC1
	    if [[ -z "$pic1_index" ]]; then
	        pic1_index=$(docker exec "$CONTAINER_NAME" grep 'PIC1_PNG' /workspace/tmpfile1 | awk '{print $4}')
	    fi
	fi
    echo "========================="
    ...
    json_entry=$(jq -n --arg title_id "$title_id" --arg name "$title" --arg version "$version" \
                      --arg release "$release" --arg size "$size" --arg cover_url "$cover_url" \
                      '{title_id: $title_id, name: $name, version: $version, release: $release, size: ($size | tonumber), cover_url: $cover_url}')
